...
// priority set and initialize the flag
    ...
    ***/
    ...
    {
        int times = 0;
        int max_index[total_vertex] = {0};
        int cmp_matrix[total_vertex] = {0}; //old
        int MIS_matrix[total_vertex] = {0};//new
        check = 0;
        ...
        ***/
        ...
        while(check ==0)
        {
            times++;// record the times to compare
            tmp_pri = 0;
            int is_in_set = 0;
            int i_old;
            for(i_old=0; i_old<total_vertex; i_old++)
            {
                ...
                for(int j=0; j<total_vertex; j++)
                {
                    ...
                    if(Nodes[i_old].neigh[j] == 1 && Nodes[j].priority >= Nodes[i_old].priority)
                    {
                        max_index[tmp_pri] = Nodes[j].index;
                        tmp_pri++;
